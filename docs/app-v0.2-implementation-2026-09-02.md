...
이 문서는 과거 v0.2 구현 기록입니다. 현재 제품 동작은 `handoff.md`와 소스 코드를
우선하며, 개별 녹음과 사전에서 도출한 측정값은 공개 문서에 포함하지 않습니다.
...
3. 사전: 용어 목록과 observed form 표시
4. 새 회의: 제목, 사전 선택, 입력·저장 정보, 녹음 시작/취소
5. 파일 가져오기: macOS native file importer 표시

과거 녹음 smoke test는 출력 파일의 개별 저장과 종료 후 Apple 한국어 전사에서
입력 출처가 유지되는지 확인하는 기능 검증이었습니다. 개별 녹음의 길이, 프레임·버퍼 수와
전사 결과는 비공개 기록에만 보존합니다. 이 검증을 장시간 녹음 안정성 보증으로 해석하지
않습니다.
